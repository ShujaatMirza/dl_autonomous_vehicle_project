...
def train(task, dataset, batch_size=2, max_training_scene=25, epochs=0, learning_rate=1e-3,
    ...
    if task == 'box':
        nn = BoundingBoxesNN(device=device, num_angles=NUM_ANGLES)
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        # Sample command: `python main.py -config config/bounding_box.config -mode train`
        train(task,
              dataset,
        ...
        # Sample command: `python main.py -config config/bounding_box.config -mode test \
